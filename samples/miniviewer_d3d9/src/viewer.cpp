...
LPDIRECT3DTEXTURE9 Viewer::loadTexture(const std::string& strFilename)
{
	...
	std::string chr = strFilename.substr(strFilename.find_last_of('.'));
	if(strcmpi(chr.c_str(),".raw")==0)
	{
		std::ifstream file;
...
bool Viewer::onCreate(LPSTR lpCmdLine)
{
  ...
  m_calModel = new CalModel();
  m_calModel->create(m_calCoreModel);
...
void Viewer::onIdle()
{
  ...
  double elapsedSeconds = Tick::TickToSecond(tick - m_lastTick);
  ...
  TICK tickUpdateStart = Tick::getTick();
  ...
  TICK tickUpdateEnd = Tick::getTick();
  m_iUpdateDuration = tickUpdateEnd - tickUpdateStart;
...
/*  if(sizeof(CalIndex)==2)
  ...
  {
	  if(FAILED(
      g_pD3DDevice->CreateIndexBuffer(50000*3*sizeof(int),
...
void Viewer::onRender()
{
  ...
  sprintf(s,"%d %lld %lld",m_fps, m_iUpdateDuration, m_iFillBufferDuration);
...
bool Viewer::parseModelConfiguration(const std::string& strFilename)
{
  ...
  if (iPos != std::string::npos)
	strPath = strFilename.substr(0, iPos);
  ...
  for(line = 1; ; line++)
  {
    ...
    else if(strKey == "skeleton")
    {
	  ...
	  strFilepath += strData;
	  std::cout << "Loading skeleton '" << strFilepath << "'..." << std::endl;

      if(!m_calCoreModel->loadCoreSkeleton(strFilepath.c_str()))
      {
    ...
    else if(strKey == "animation")
    {
      // load core animation
      std::string strFilepath = strPath + "/";
      strFilepath += strData;      std::cout << "Loading animation '" << strFilepath << "'..." << std::endl;
      if(m_calCoreModel->loadCoreAnimation(strFilepath.c_str()) == -1)
      {
        CalError::printLastError();
        return false;
      }
    ...
    else if(strKey == "mesh")
    {
      // load core mesh
		std::string strFilepath = strPath + "/";
		strFilepath += strData;      std::cout << "Loading mesh '" << strFilepath << "'..." << std::endl;
      if(m_calCoreModel->loadCoreMesh(strFilepath.c_str()) == -1)
      {
        CalError::printLastError();
        return false;
      }
    ...
    else if(strKey == "material")
    {
      // load core material
		std::string strFilepath = strPath + "/";
		strFilepath += strData;      
		std::cout << "Loading material '" << strFilepath << "'..." << std::endl;
      if(m_calCoreModel->loadCoreMaterial(strFilepath.c_str()) == -1)
      {
        CalError::printLastError();
        return false;
      }
  ...
  for(materialId = 0; materialId < m_calCoreModel->getCoreMaterialCount(); materialId++)
  {
	  ...
	  for(mapId = 0; mapId < pCoreMaterial->getMapCount(); mapId++)
	  {
		  ...
		  LPDIRECT3DTEXTURE9 texture;
		  texture = loadTexture(strFilepath);
  ...
  for(meshId = 0; meshId < meshCount; meshId++)
  {
    ...
    for(submeshId = 0; submeshId < submeshCount; submeshId++)
    {
      ...
      if(pCalRenderer->selectMeshSubmesh(meshId, submeshId))
      {
		  ...
		  TICK iTickFillStart = Tick::getTick();
		  ...
		  int vertexCount = pCalRenderer->getVertices(&pVertices->pos.x, VERTEX_XYZ | VERTEX_NORMAL | VERTEX_TEX1);
		  m_pVB->Unlock();
		  ...
		  m_pIB->Lock(m_IBCursor* 3*sizeof(int), pCalRenderer->getFaceCount()*3* sizeof(int), (void**)&meshFaces,dwIBLockFlags);

		  faceCount = pCalRenderer->getFaces(meshFaces);
		  m_pIB->Unlock();
		  TICK iTickFillEnd = Tick::getTick();
		  m_iFillBufferDuration += iTickFillEnd - iTickFillStart;
