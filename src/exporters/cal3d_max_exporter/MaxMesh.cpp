...
#include "../src/SkeletonCandidate.h"
#include "../src/VertexCandidate.h"
...
int CMaxMesh::GetFaceMaterialId(unsigned int faceId)
{
	// check for valid mesh
	if(m_pIMesh == 0)
	{
...
int CMaxMesh::GetSubmeshMapCount(unsigned int submeshId)
{
...
int CMaxMesh::GetSubmeshMaterialThreadId(unsigned int submeshId)
{
	// check if the submesh id is valid
	if((submeshId < 0) || (submeshId >= m_vectorStdMat.size()))
	{
